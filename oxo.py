...
def placedot(board,placer):
  while True:
    if placesLeft(board) != 0:
      ...
      if xcoord in acceptable_values and ycoord in    acceptable_values and area_available:
        board[xcoord-1][ycoord-1] = dot
        return board
      elif xcoord not in acceptable_values or ycoord not in   acceptable_values:
        print(f"Please re-enter values: either {xcoord} not allowed or {ycoord} not allowed")
    else:
      return False
